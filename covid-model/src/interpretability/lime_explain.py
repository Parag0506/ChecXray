...
def explain_xray(lime_dict, idx, save_exp=True):
    ...
    print(lime_dict['RAW_DATA_PATH'] + lime_dict['TEST_SET']['filename'][idx])
    print(lime_dict['TEST_SET']['filename'][idx].split('/'))
    o = lime_dict['TEST_SET']['filename'][idx].split('/')[1]
    if (os.path.exists('./data/processed/test/'+ o)):
        orig_img = cv2.imread('data/processed/test/' + o)
...
if __name__ == '__main__':
    ...
    for i in range(0, 100):                                                    # Select i'th image in test set
